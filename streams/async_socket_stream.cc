...
bool AsyncSocketStream::start() {
    // 如果 IO 管理器为空，则获取当前线程的 IO 管理器
    if(!m_iomanager) {
        m_iomanager = yk::IOManager::GetThis();
    }
    // 如果工作线程管理器为空，则获取当前线程的 IO 管理器
    if(!m_worker) {
        m_worker = yk::IOManager::GetThis();
    }
    ...
    // 如果设置了自动连接，则添加定时器重新启动异步 Socket 流
    if(m_autoConnect) {
        if(m_timer) {
            m_timer->cancel();
            m_timer = nullptr;
...
void AsyncSocketStream::doRead() {
    ...
    YK_LOG_DEBUG(g_logger) << "doRead out " << this;
    innerClose();
    m_waitSem.notify();
...
void AsyncSocketStream::doWrite() {
    ...
    YK_LOG_DEBUG(g_logger) << "doWrite out " << this;
    {
        RWMutexType::WriteLock lock(m_queueMutex);
        m_queue.clear();
    }
...
bool AsyncSocketStream::innerClose() {
    YK_ASSERT1(m_iomanager == yk::IOManager::GetThis());
